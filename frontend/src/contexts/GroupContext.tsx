...
export const GroupContextProvider = ({ children }: GroupContextProps) => {
    ...
    const updateGroup = useCallback(async (newGroupDetails: Partial<Group>) => {
        if (isAuthenticated) {
            setGroup((prevState) => ({ ...prevState, ...newGroupDetails }));

            // Save what will be the new group to local storage
            localStorage.setItem('group', JSON.stringify({ ...group, ...newGroupDetails }));
        }
    ...
    useEffect(() => {
        async function fetchUserGroups() {
            ...
            if (isAuthenticated) {
                try {
                    // Replace '/api/user-groups' with your actual API endpoint
                    const response = await authedFetch('/api/groups/user');
                    if (!response.ok) {
                        throw new Error('Failed to fetch user groups');
                    }
                    const userGroups: Group[] = await response.json();
                    // console.log("userGroups: " + userGroups[0].description);
        ...
        //Read the group from local storage
